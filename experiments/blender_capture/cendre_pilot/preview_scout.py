"""Scout render: import cendre_combo.glb once, render a coarse evenly-
spaced set of frames across the Punch_Combo action at low res/samples,
to visually locate contact/anticipation/recovery beats before building
the final weighted non-uniform sampling. Diagnostic only, not part of
the final pilot deliverable."""
import bpy
import sys
import os
import math
...
n_frames = int(args.get("n_frames", "24"))
res = int(args.get("res", "160"))
...
yaw, pitch = math.radians(float(args.get("yaw_deg", "35"))), math.radians(18)
...
# Fixed common target_z computed once from a mid-pose bbox so the whole
# scout strip shares one framing (avoids jitter between frames).
...
for i in range(n_frames):
    t = i / (n_frames - 1)
    frame = frame_start + t * (frame_end - frame_start)
    ...
    out_path = os.path.join(out_dir, f"scout_{i:02d}_f{frame:.1f}.png")
    scene.render.filepath = out_path
    bpy.ops.render.render(write_still=True)
    print("SCOUT_RENDERED", i, frame, out_path)

print("SCOUT_DONE", n_frames)
